...
// `typeNotif`, si fourni, est le nom d'une colonne booléenne sur `employes`
// (ex: "notif_messages") - seuls les employés qui l'ont encore à true
// reçoivent la notification (voir ParametresEmploye.jsx > Notifications).
export async function envoyerPushEmployes(service, employeIds, { titre, corps, url }, typeNotif) {
  ensureConfigured();
  if (!process.env.VAPID_PRIVATE_KEY || employeIds.length === 0) return;

  let idsAutorises = employeIds;
  if (typeNotif) {
    const { data: employesAutorises } = await service
      .from("employes")
      .select("id")
      .in("id", employeIds)
      .eq(typeNotif, true);
    idsAutorises = (employesAutorises || []).map((e) => e.id);
  }
